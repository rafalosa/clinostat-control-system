        ...
        while True:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sc:
                ...
                try:
                    ...
                    time.sleep(5)
                    break  # Here interrupt the forever loop and continue to watch fot the connection.

                current_timestamp = time.time()
                if current_timestamp - last_measurement_timestamp >= 0.5 * 60:
                    print("Scheduling saturation measurement.")
                    Thread(target=measurement_dummy, args=(saturation_queue,))
                    last_measurement_timestamp = current_timestamp

                accel_values = grav_sensor.read()
                accel_values = [val / (2 ** 16 / 4) for val in accel_values]
